export default function IconArrow({
  direction = "down",
  width = 18,
  height = 18,
  className = "",
}) {
  const rotationClass = {
    right: "rotate-0",
    left: "rotate-180",
  }[direction];

  return (
    <svg 
    fill="current" height="800px" width="800px"viewBox="0 0 512 512"
    className={`fill-current text-navy transform ${rotationClass} transition-all duration-300 ${className}`}    
    >
    <g><g><g>
                <path d="M256,0C114.618,0,0,114.618,0,256s114.618,256,256,256s256-114.618,256-256S397.382,0,256,0z M256,469.333
                    c-117.818,0-213.333-95.515-213.333-213.333S138.182,42.667,256,42.667S469.333,138.182,469.333,256S373.818,469.333,256,469.333
                    z"/>
                <path d="M228.418,134.248c-8.331-8.331-21.839-8.331-30.17,0c-8.331,8.331-8.331,21.839,0,30.17L289.83,256l-91.582,91.582
                    c-8.331,8.331-8.331,21.839,0,30.17c8.331,8.331,21.839,8.331,30.17,0l106.667-106.667c8.331-8.331,8.331-21.839,0-30.17
                    L228.418,134.248z"/>
            </g>
        </g>
    </g>
    </svg>
  );
}
